...
class Mvp extends Component {
  ...
  initialiseDapp = async () => {
    const intialisation = await this.props.initialiseWeb3();
    const { networkId, web3, accounts } = await window.store.getState();
    const validationContract = Validation.networks[networkId];
    const faucetContract = Faucet.networks[networkId];
    const tokenContract = ERC20d.networks[networkId];

    const validationInstance = new web3.eth.Contract(Validation.abi,
           validationContract && validationContract.adress,
    ); const tokenInstance = new web3.eth.Contract(ERC20d.abi,
      tokenContract && tokenContract.address,
    ); const faucetInstance = new web3.eth.Contract(Faucet.abi,
      ...
      : this.renderSidebar;
       if(window.screen.height < 800 && window.screen.width < 950){
        ...
        <div className="errorModal">
          <div className="conditioningTitle">Not supported natively</div>
           <Error />
        </div>
        )
        ...
        return(
          <div className="errorModal">
            <div className="conditioningTitle">No Web3 instance detected</div>
            <Error />
          </div>
        )
        ...
        return(
        <div className="errorModal">
          <div className="conditioningTitle">Incorrect network, Rinkeby only</div>
          <Error />
        </div>
        )
      }
